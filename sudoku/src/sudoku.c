...
typedef enum { mode_first, mode_all, mode_generate, mode_unique } mode_t;
...
/************ GRID_BACKTRACK ************/
static grid_t *grid_backtrack (grid_t *grid, mode_t mode, FILE *output_file) {
...
grid_t *grid_generate (size_t size, bool unique) {
  ...
  size_t nb_empty_cell = size * size * 25 / 100;
